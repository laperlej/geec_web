$(document).ready(function () {
  ...
  var main_table = $("#bw-table").DataTable({
    ...
    fnInitComplete: function (oSettings, json) {
      ...
      all_checkboxes = $(
        ":checkbox",
        main_table.rows({ filter: "applied" }).nodes()
      );
    },
  });
  ...
  function selectAll() {
    if (
      main_table.page.info().recordsDisplay ==
      main_table.page.info().recordsTotal
    ) {
      all_checkboxes.prop("checked", true);
    } else {
      $(":checkbox", main_table.rows({ filter: "applied" }).nodes()).prop(
        "checked",
        true
      );
    }
  ...
  */
  ...
  function updateSelectionOptions(column_idx) {
    var new_options = main_table
      .column(column_idx, { search: "applied" })
      .data()
      .unique()
      .sort();
